...
# Configure application with correct paths for Vercel
app = Flask(__name__, 
...
def disease():
    ...
    if request.method == "POST":
        ...
        json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'functions', 'plant_diseases.json')
        try:
...
def weeddetection():
    ...
    if request.method == "POST":
        ...
        json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'functions', 'weed_dataset.json')
        try:
            with open(json_path, 'r') as f:
